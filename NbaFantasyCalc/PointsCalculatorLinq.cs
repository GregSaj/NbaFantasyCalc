...
{
    public class PointsCalculatorLinq
    {
        ...
        {            
            return players.Where(p => p.Name == playerName).Select(x => x.num3P).Sum();
        }

        public static double ReboundsCalc(List<Player> players, Player player)
        {
            return players.Where(p => p.Name == player.Name).Select(x => x.TRB).Sum() * 1.2;
        }
